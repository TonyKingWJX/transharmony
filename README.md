...
- ✅ 应用内备份与恢复：全部历史（含回收站）+ 服务商配置（含密钥）+ 偏好设置 + **可选的已安装插件（文件/凭证/配置）**，**密码可选**（v2 格式 THBK2：有密码 PBKDF2 派生 AES-256-GCM 整包加密，无密码固定常量派生仅保完整性、弹窗醒目警示），落 Download/<包名>/ 免权限；无密码备份恢复免输密码、旧 THBK1 备份兼容；图片原图不再随备份携带（详情页回退 meta 缩略图）；恢复支持历史合并去重/覆盖、文件选择器拉起
