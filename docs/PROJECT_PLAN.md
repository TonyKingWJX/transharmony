# 鸿译 TransHarmony 项目规划（内部文档）

> 本文档是项目的完整内部规划与开发记录（原开发版 README 全文存档），包含产品定位、商业模式、排期、待决策清单、工程现状与已知待办。对外介绍见仓库根目录 [README](../README.md)。
...
**当前状态：v1.0.1，功能开发完成（含 Bob 插件系统），真机可跑，待上架准备（商店素材、AGC 配置）。** 详见「11. 工程现状」与「12. 版本历史」。
...
- [ ] **AI 引擎无流式输出**：`translateStream`（SSE）未实现，当前一次性返回
- [x] ~~「插件翻译服务」入口已预留，暂未开放~~ ✅ 已上线（v1.0.1）：Bob 插件系统全量落地
- [ ] **插件：流式与取消**：$http.streamRequest/streamHandler + query.onStream + cancelSignal（HttpClient 需支持 SSE/分块）——OpenAI 类插件的打字机体验
- [ ] **插件：$websocket 桥接**：部分 AI/实时翻译服务使用（@ohos.net.websocket 可桥接）
...
- [ ] **插件：ESM 规范（Manggo）加载层**：JSVM 无 ES Module 支持，需模块转换层（推荐，1~2 天）或内嵌 QuickJS；.mplugin 多服务模型拆分；来源标识已预留（source=manggo）。⚠️ 约束：不得读写用户桌面安装的 Manggo 程序及其目录，测试用 .mplugin 样本一律由用户提供
- [ ] **插件：appcast 自动更新、info.json 多图标/isKeyOption** 未处理
- [ ] **插件：诊断日志降级**：实验室调试页（PluginLabPage）与内置 mock.bobplugin 上架前移除/隐藏
